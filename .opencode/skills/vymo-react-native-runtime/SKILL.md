...
4. Store runtime artifacts only under `./tmp/{ticketKey}/{platform}/`.
5. Before writing any artifact files, create the ticket temp tree with `scripts/create-session-dirs.sh`.
...
8. If Metro is not running, start it in the background and keep all logs and pidfiles inside the repo-local ticket temp tree.
...
- Use `./tmp/{ticketKey}/ios/...` for iOS runs.
- Use `./tmp/{ticketKey}/android/...` for Android runs.
...
- Prefer the bundled temp-dir helper over ad hoc `mkdir` calls when creating the ticket tree.

## Background Process Rules

- First check existing Metro state with `scripts/check-metro.sh`.
- First ensure the temp ticket tree exists with `scripts/create-session-dirs.sh`.
...
- Export or pass `TICKET_KEY` and `PLATFORM` before invoking the scripts so the temp path and app root land in the correct ticket tree.
